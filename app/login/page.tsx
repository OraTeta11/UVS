...
export default function LoginPage() {
  ...
  const handleNextStep = async () => {
    ...
    try {
      // Fetch the stored face descriptor for the student ID
      const response = await fetch(`/api/users?studentId=${studentId}`);
      if (!response.ok) throw new Error("Failed to fetch user data");
      const data = await response.json();
      if (!data.faceDescriptor) throw new Error("No face data registered for this user");
      setStoredFaceDescriptor(new Float32Array(data.faceDescriptor));
      setIsVerifying(false)
      setStep(2)
    } catch (error: any) {
      setLoginError(error.message || "Failed to verify student ID. Please try again.");
      setIsVerifying(false)
    }
  }
  ...
  const handleFaceVerificationComplete = async () => {
    if (!capturedFaceDescriptor) {
      setLoginError("Face data not captured. Please try again.");
      setVerificationComplete(false);
      return;
    }
    if (!studentId) {
            ...
            {step === 2 && (
              <div className="space-y-6">
                ...
                <FaceVerification
                  ...
                  storedFaceDescriptor={storedFaceDescriptor ?? undefined}
                />
