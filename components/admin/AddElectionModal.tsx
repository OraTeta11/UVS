...
interface AddElectionModalProps {
  isOpen: boolean;
  onClose: () => void;
  onAdd: (newElection: Partial<Election>) => void;
}
...
export function AddElectionModal({
  ...
  const handlePositionsChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Split by comma and trim whitespace for multiple positions
    setFormData((prev) => ({ ...prev, positions: value.split(',').map(p => p.trim()) }));
  };
