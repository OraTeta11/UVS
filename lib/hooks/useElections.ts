...
export const useElections = (status?: 'active' | 'upcoming' | 'past') => {
  ...
  const fetchElections = async () => {
    try {
      setLoading(true);
      setError(null);
      const data = await electionService.getElections(status);
      setElections(data);
    } catch (err) {
...
export const useElection = (id: string) => {
  ...
  const submitVote = async (candidateId: string, position: string) => {
    try {
      setLoading(true);
      setError(null);
      await electionService.submitVote({ electionId: id, candidateId, position });
