...
export const electionService = {
  getElections: async (status?: 'active' | 'upcoming' | 'past'): Promise<Election[]> => {
    try {
      ...
      const data = await api.get<Election[]>(endpoint);
  ...
  submitVote: async (voteData: VoteData): Promise<{ success: boolean }> => {
    try {
      const data = await api.post<{ success: boolean }>(`/elections/${voteData.electionId}/vote`, voteData);
      return data;
    } catch (error) {
